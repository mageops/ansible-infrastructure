...
# Local change: validate_origins keys explicit origins by CloudFront origin id
# instead of domain name so same-domain origins stay distinct. Id-less origins
# still match existing origins by domain plus origin_path.
# Based on ansible/ansible#68355 for ansible/ansible#68354.
...
class CloudFrontValidationManager(object):
    ...
    def validate_origins(
        ...
        try:
            ...
            def get_origin_fallback_key(origin, implied_origin_path=""):
                return origin.get("domain_name") + (origin.get("origin_path") or implied_origin_path or "")

            def get_origin_key(origin):
                return origin.get("id") if "id" in origin else get_origin_fallback_key(origin)

            new_origins = list()
            existing_origins_by_fallback_key = {}
            for origin in config:
                all_origins[get_origin_key(origin)] = origin
                existing_origins_by_fallback_key[get_origin_fallback_key(origin)] = origin
            for origin in origins:
                if "id" in origin:
                    existing_origin = all_origins.get(get_origin_key(origin), {})
                else:
                    desired_origin_key = get_origin_fallback_key(origin, default_origin_path)
                    existing_origin = existing_origins_by_fallback_key.get(desired_origin_key, {})
                origin = self.validate_origin(
                    client, existing_origin, origin, default_origin_path
                )
                all_origins[get_origin_key(origin)] = origin
