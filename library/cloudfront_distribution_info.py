...
# Local fork of community.aws 11.0.0.
# Local change: build summary data here with keyword ID arguments instead of
# calling CloudFrontFactsServiceManager.summary(), which still calls
# get_origin_access_identity() with a positional ID and fails with amazon.aws
# 11.0.0.
...
def get_cloudfront_origin_access_identity_summary(service_mgr):
    origin_access_identities = []
    for origin_access_identity in service_mgr.list_origin_access_identities():
        oai_id = origin_access_identity["Id"]
        oai_full_response = service_mgr.get_origin_access_identity(id=oai_id)
        origin_access_identities.append({"Id": oai_id, "ETag": oai_full_response["ETag"]})
    return {"origin_access_identities": origin_access_identities}


def get_cloudfront_distribution_summary(service_mgr, streaming=False):
    ...
    distributions = service_mgr.list_streaming_distributions(keyed=False) if streaming else service_mgr.list_distributions(keyed=False)

    for dist in distributions:
        temp_distribution = {key: dist[key] for key in key_list}
        temp_distribution["Aliases"] = list(dist["Aliases"].get("Items", []))
        distribution = service_mgr.get_streaming_distribution(id=dist["Id"]) if streaming else service_mgr.get_distribution(id=dist["Id"])
        temp_distribution["ETag"] = distribution["ETag"]

        if not streaming:
            temp_distribution["WebACLId"] = dist["WebACLId"]
            invalidation_ids = [invalidation["Id"] for invalidation in service_mgr.list_invalidations(distribution_id=dist["Id"])]
            if invalidation_ids:
                temp_distribution["Invalidations"] = invalidation_ids

        resource_tags = service_mgr.list_resource_tags(dist["ARN"])
        temp_distribution["Tags"] = {
            tag["Key"]: tag["Value"] for tag in resource_tags["Tags"].get("Items", [])
        }
...
def get_cloudfront_summary(service_mgr):
    summary = {}
    summary.update(get_cloudfront_distribution_summary(service_mgr, streaming=False))
    summary.update(get_cloudfront_distribution_summary(service_mgr, streaming=True))
    summary.update(get_cloudfront_origin_access_identity_summary(service_mgr))
    return summary
...
def main():
    ...
    if summary:
        facts["summary"] = get_cloudfront_summary(service_mgr)
